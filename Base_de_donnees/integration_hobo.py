...
def integration_hobo(ficjson):
    ...
    with open(fichiercsv, newline="", encoding="utf-8") as f:
        ...
        if capt_loc == []:
            ...
            """, (data["extension"], nom_fic, data["chemin_source"], data["date_import"], format_timestamp(data["date_import"]), data["date_collecte"], format_timestamp(data["date_collecte"]), data["commentaire"], id_responsable_fic, data["num_instrument"], "fichier_mesure", data["extension"], nom_fic, data["chemin_source"], data["date_import"], format_timestamp(data["date_import"]), data["date_collecte"], format_timestamp(data["date_collecte"]), data["commentaire"], id_responsable_fic, data["num_instrument"], "fichier_mesure"))

            cur.execute("SELECT id_source FROM source_donnees WHERE lower(extension) = lower(%s) AND lower(nom_source) = lower(%s) AND lower(chemin_source) = lower(%s) AND date_import = to_timestamp(%s, %s) AND date_collecte = to_timestamp(%s, %s) AND lower(commentaire) = lower(%s) AND id_responsable = %s AND id_struct = (SELECT s.id_structure FROM structure_fichier s JOIN instrument_mesure im ON lower(im.nom_outil) = lower(s.nom_instrument) WHERE lower(im.num_instrument) = lower(%s)) AND lower(type_source) = lower(%s) LIMIT 1;", (data["extension"], nom_fic, data["chemin_source"], data["date_import"], format_timestamp(data["date_import"]), data["date_collecte"], format_timestamp(data["date_collecte"]), data["commentaire"], id_responsable_fic, data["num_instrument"], "fichier_mesure"))
